...
The Watch Desk is scheduled every 15 minutes (`WATCHDESK_EVAL_INTERVAL_MINUTES = 15`), but that is the configured interval, not the cadence (corrected 2026-09-12). A sweep of ~162 watches overruns its own interval and APScheduler's default `max_instances=1` drops roughly 2 of every 3 fires — 21 `skipped: maximum number of running instances reached` warnings in a single afternoon — so the measured period is about 45 minutes and tuning the parameter below that changes nothing. Nor does the poll rate set how often the desk wakes a cycle: wakes are admitted by a `score_floor` (`watch_triage.score_floor`) that rises from 1.2 at a full budget to 3.0 at the last wake as the DAILY budget is spent, so a trip must beat a bar that gets higher through the day. Live enforcement is enabled (`WATCH_TRIAGE_MODE=2`), with a 6-wake daily budget, 12-hour per-ticker analysis cadence and 3 analyses per ticker per week. Failed cycles that produced no analysis can be refunded, so 6 is not a hard ceiling on attempted starts. The bounded agent planner is disabled (`WATCH_PLANNER_ENABLED=0`); the dispatch code does not currently call its recommendation function. Comments saying shadow is the default are stale relative to the registry and live records.
