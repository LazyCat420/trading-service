...
Targeted deploy-kit deployment of `trading-service` completed successfully in about eight minutes. Image transfer and restart succeeded; the running container is healthy at revision `521bc3fe`, and `/health` returned HTTP 200. Read-only checks inside the running container verified named-envelope recovery, preserved research answers, unchanged entry rejection, and rejection of altered answers/questions. Both production source hashes match the validated local files. The pipeline was idle before the swap. The client and adapter repositories are unchanged. Deploy-kit also reported existing edge DNS conflicts; backend transfer, restart and direct HTTP health succeeded. See [release verification](evidence/board-failure-isolation-2026-09-11/release-verification.json).


Subsequent improvement: [unique timing-label normalization](board-timing-normalization-2026-09-11.md) reached 5/5 structurally in two further replays. The results above remain the historical pre-normalization audit.
