...
⚠ The `description=` strings below are NOT what the model reads.
`app/tools/registry.py:_load_scoped_schemas` loads `tool_schemas.json` over
this registration, and that file is a gitignored BUILD ARTIFACT generated by
`scripts/build_tool_schemas.py` from the split source at
`lazy-agent-service/tool_schemas/<owner_app>/<domain>.json`.

Measured 2026-07-29: editing a `description=` here and reloading left the live
schema unchanged. To change what an agent actually sees, edit the split source
and re-run the build (it writes byte-identical copies to lazy-agent-service,
trading-service and trading-client — asserted by tests/test_multi_repo_audit.py).
The strings here are kept in sync anyway, because they are the fallback when the
artifact is absent and because a decorator that contradicts the live schema is a
trap for the next reader. Pinned by tests/unit/test_sandbox_stat_helpers.py,
which asserts against the LIVE registry rather than this source.
"""
    ...
    parameters={
        ...
        "properties": {
            ...
            "code": {
                "type": "string",
                "description": (
                    "Python code. Must assign output to 'result'. Has access to "
                    "df, params, np, pd, plus statistics helpers: "
                    "newey_west_tstat(returns), stationary_bootstrap_ci(returns), "
                    "probabilistic_sharpe_ratio(returns), "
                    "deflated_sharpe_ratio(returns, n_trials), "
                    "min_track_record_length(returns), is_oos_degradation(returns), "
                    "full_gate(returns), suggest_lag(n), norm_cdf(x). "
                    "scipy is NOT importable — use these instead."
                ),
            },
            "description": {
                "type": "string",
...
@registry.register(
    ...
    description=(
        ...
        "Statistics helpers are pre-injected (scipy is NOT importable): "
        "newey_west_tstat(returns) for an autocorrelation-robust t-stat, "
        "stationary_bootstrap_ci(returns) for a block-bootstrap CI, "
        "probabilistic_sharpe_ratio(returns) and "
        "deflated_sharpe_ratio(returns, n_trials) to correct a Sharpe for skew, "
        "kurtosis, sample length and the number of variants tried, "
        "min_track_record_length(returns), is_oos_degradation(returns), "
        "full_gate(returns) to run all three at once, suggest_lag(n), norm_cdf(x). "
        "Use deflated_sharpe_ratio whenever you compare several equations — "
        "the best of N tried is inflated by chance."
    ),
    parameters={
        "type": "object",
        "properties": {
            ...
            "code": {
                "type": "string",
                "description": (
                    "Raw Python code if not using a named equation. Has df, params, "
                    "np, pd and the statistics helpers listed in this tool's "
                    "description."
                ),
            },
            "parameters": {
