...
# The per-column money policy lives in `money_policy.py`, which trading-client
# carries byte-identically (see scripts/check_shared_files.py). It is
# re-exported here because this module is what the backfill and the specs
# import, and because `column_is_money` used to be defined here — a second
# definition in the client would be a second answer to "is this column money",
# and the two halves of the contract disagreeing about one column is exactly
# the defect the policy exists to prevent.
from app.db.money_policy import (  # noqa: E402,F401
    _MONEY_COLUMNS,
    _NON_MONEY_COLUMNS,
    _QUANTITY_COLUMNS,
    column_is_money,
