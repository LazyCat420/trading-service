...
class PipelineService:
    ...
    async def _run_all_v3(cls, cycle_id: str, tickers: list[str], max_tickers: int | None = None, agent_locale: str = "default", **kwargs):
        ...
        def _persist_summary(status: str, tickers_final, results=None, error: str | None = None,
            ...
            try:
                ...
                summary = {
                    ...
                    "model_discovery": cls._state.get("model_discovery"),
        ...
        try:
            ...
            cls._state["model_discovery"] = discovery
            cls.emit("starting", "MODEL_DISCOVERY", "Verified serving models and capabilities",
                     data={"kind": "model_discovery", **discovery})
            _llm_ok = discovery["eligible"]
            _llm_detail = str(discovery)
        except Exception as _pf_err:
            _llm_ok, _llm_detail = False, f"model discovery unavailable: {type(_pf_err).__name__}"
        if not _llm_ok:
