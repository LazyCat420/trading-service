...
All backend code uses these functions to resolve the active bot,
instead of reading settings.BOT_ID directly. Pure MongoDB implementation.
...
def set_active_bot(bot_id: str) -> None:
    ...
    exists = mongo_query.find_row('bots', {'bot_id': bot_id}, ['bot_id'])
    if not exists:
    ...
    return [
        {
            ...
            "last_run_at": r[11].isoformat() if hasattr(r[11], "isoformat") else str(r[11]) if r[11] else None,
            "description": r[12] or "",
        }
        for r in rows
    ]
