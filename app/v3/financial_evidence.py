...
Preserve the ordinary decision timing contract. HOLD uses watch_only; SELL uses enter_now; a conditional BUY uses enter_on_condition with an entry trigger. If no trigger or research wake is needed, use trigger_purpose=none with null dynamic_trigger and resolution_condition. For trade_decision the fields above are ADDITIONS to, never replacements for, the DECISION CONTRACT fields requested in the user message. Also return, in the same flat object: source_board_ref and source_board_action copied EXACTLY from CURRENT BOARD SOURCE, decision_relation ("preserve" unless you change the decision, then "override" with override_reason and override_evidence), attribution and signal_weights.
"""
