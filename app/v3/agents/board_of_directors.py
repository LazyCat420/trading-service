...
- If you like the fundamental setup but current entry price is extended, do NOT default to passive HOLD: output `action: "BUY"`, calibrate `position_size_pct` (e.g. 1.5-2.5%), and specify a `dynamic_trigger` (e.g. `rsi_14_oversold` or `sma_50_drop`) with entry_mode="enter_on_condition" and trigger_purpose="entry" so the system waits and re-analyzes when the level is reached. If entry is appropriate now and the trigger only monitors later changes, use entry_mode="enter_now" and trigger_purpose="monitor".
