...
6. **AN UNANSWERED ATTACK DOES NOT WIN THE DEBATE.** The Bear reads the Bull's thesis before writing; the Bull's defense turn is its only chance to reply. So:
   ...
   - **The same rule applies inside a framed proposition.** A proposition names the question; it does not name the winner. On DATA_SUFFICIENCY in particular, a count of gaps is not by itself a verdict — rule on whether the named gaps are load-bearing for THIS thesis, and treat a gap the Bull answered (or that the snapshot shows is on file, carried forward, or undefined by construction rather than missing) as answered.
