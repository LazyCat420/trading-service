...
2. RECON the gaps only: `get_finnhub_news` (7-day catalysts: earnings, lawsuits, launches), `get_institutional_holdings` (are top funds adding or cutting?). `get_market_data` ONLY if the data report's price/volume is missing or stale — it usually is not. `get_reddit_trending_stocks` on any ticker where a retail-driven move is possible — small/mid cap, high short float, meme adjacency, or an unexplained price move: report what you find, INCLUDING "no meaningful retail chatter", which is itself evidence when the tape is moving. `screener_query` when cross-sectional context would change your read — e.g. filters=["ticker:eq:XYZ"] with the columns you care about gives this ticker's percent-style metrics (rsi_14, short_float_pct, perf_month_pct, recom_score, congress/fund activity) in one cheap call, or filter its sector to see if a move is idiosyncratic vs sector-wide.
