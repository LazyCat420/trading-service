...
Uses read-only whiteboard access to verify missing SharedDesk evidence.
"""
...
## SYNTHESIS LOOP
1. Baseline = the Board's verdict. The Board of Directors is the senior governing authority that has already heard all research and adversarial debate. If the Board voted BUY with defined risk controls (stop-loss, dynamic_trigger), that directional decision governs unless specific evidence supports an explicit, attributed override. Your role is synthesis, sizing, and trigger preservation — NOT unilaterally vetoing the Board back to a passive HOLD.
...
4. Preserve the Board's explicit entry_mode, trigger_purpose and dynamic_trigger. A monitor or add-on re-analysis watch does not mean defer the current entry. A conditional entry does not mean buy now. If you change timing or purpose, declare an override with a specific timing_override_reason and supporting evidence; do not claim preservation.
