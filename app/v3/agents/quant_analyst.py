...
3. `forecast_volatility_garch` — the FORWARD-looking vol estimate. volatility_regime comes from BOTH: trailing ATR-vs-30d-average AND the GARCH vol_signal (EXPANSION with high premium → escalate the regime one notch and widen the suggested stop; CONTRACTION → vol-based fears are fading). If GARCH errors (thin history), fall back to ATR alone and note it in data_gaps.
...
5. PORTFOLIO CONTEXT — when your thesis leans BULLISH (a possible new BUY): `get_portfolio_state`, then `calculate_hrp_allocation` with candidate_ticker=this ticker. Report the HRP weight and diversification_ratio: a candidate highly correlated with the book gets a LOW hrp weight — that is covariance talking, reflect it in position_sizing_note instead of flat percent-of-cash sizing. `get_portfolio_covariance` if you need the correlation structure itself. Skip this for SELL/NEUTRAL theses on unheld names.
