...
3. FILL gaps only as needed: `get_sec_filings` (debt/balance sheet), `get_institutional_holdings` (ownership trend), `get_finnhub_news`/`lazy_web_search` (verify a specific catalyst — no general browsing). If a needed metric is still missing, record it as `DataGap: <metric>` — the Junior has already finished and this panel has no direct peer-request tool. Record the gap for downstream review.
...
5. `whiteboard_annotate` — at least once: use the Junior's "desk_note" or "market_context" entry_id already supplied (read only if the needed entry is omitted), and annotate it with ONE line: AGREE or DISPUTE + the number that supports you. Pass author="v3_fundamental_analyst". Quant normally runs after you, so do not poll for its "signals" section. A missing teammate entry is a data gap; do not invent an entry_id or a disagreement.
6. Emit the JSON.
...
- Do NOT use `scrape_url` on anti-bot/Cloudflare protected domains (e.g. seekingalpha.com, finance.yahoo.com, bloomberg.com) — they return HTTP 403. Rely on the PRECOMPUTED FUNDAMENTAL SNAPSHOT, SEC/EDGAR tools, or record `DataGap: <unreachable source>` instead of retrying blocked scrapers.
